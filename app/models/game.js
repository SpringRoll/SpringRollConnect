...
GameSchema.statics.getBySlugOrBundleId = function(slugOrBundleId, callback) {
  return this.findOne(
    { $or: [{ slug: slugOrBundleId }, { bundleId: slugOrBundleId }] },
    callback
  );
};
...
GameSchema.methods.hasGroup = function(group) {
  return (
    group.privilege ||
    this.groups.find(function(entry) {
      // If the groups are populated
      if (entry.group._id) return entry.group._id.equals(group._id);
      else return entry.group.equals(group._id);
    })
  );
};
