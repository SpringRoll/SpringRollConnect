...
(function(){
	...
	// IE 7 Fallback for localStorage using cookies
...
(function(){
	...
	// assign to namespace
	namespace('pbskids').Platform = Platform;
...
(function(undefined){
	...
	var localStorage = include('localStorage'),
		UUID = include('pbskids.UUID'),
		$ = include('PBS.KIDS.$_auth', false);
	...
	p._onTimerTick = function()
	{
		...
		{
			if (this.processBatch(localStorage.key(i)))
			{
				break;
			}
		}
	...
	p.flushAll = function()
	{
		...
		{ 
			...
			for (var i = 0; i < localStorage.length; i++)
			{
				this.processBatch(localStorage.key(i));
			}
		}             
	};
	...
	// Assign to namespace
	namespace('pbskids').BufferedQueue = BufferedQueue;
...
(function(){
	...
	// Import classes
	var UUID = include('pbskids.UUID'),
		BufferedQueue = include('pbskids.BufferedQueue'),
		Platform = include('pbskids.Platform'),
	...
	/**
	 * Send the progress tracker events
	 * @class Tracker
	 * @constructor
	 * @param {object} [options] The list of configuration options
	 * @param {string} [options.domain=null] The domain for the end-point
	 ...
	 * @param {string} [options.resource='game'] The type of resource
	 * @param {string} [options.remoteHost] The hostname or IP Address of the machine
	 *        to log out the event data through WebSocket connection.
	 * @param {string} [options.remotePort=1025] The port for remote logging using
	...
	var Tracker = function(options)
	{
		...
		this.queue = new BufferedQueue(
			BATCH_SIZE,
			KEY_PREFIX, 
			this.options.domain + servicePath
		);
	 ...
	 */
	p.remoteConnect = function(host, port)
	{
		...
		if (("WebSocket" in window || "MozWebSocket" in window) && host) 
		{
			...
			// Bind handlers
			onCloseRemote = onCloseRemote.bind(this);
			onOpenRemote = onOpenRemote.bind(this);
	 ...
	 */
	p.remoteSend = function(type, data)
	{
		var event = {
			type: type,
	 ...
	 * @method remoteChannel
	 * @param {string} channel The channel name
	 */
	p.remoteChannel = function(channel)
	{
		this._channel = channel;
		this.remoteSend('session');
	};
	 ...
	 * @method setEnabled
	 * @param {boolean} enabled If we're enabled
	 */
	 ...
	 * @return {pbskids.Tracker} Instance for chaining
	 */
	...
	{
		this.remoteSend('pt-event', eventData);
		...
		if (Identity)
		{
			...
			{
				eventData.user_ids[i] = users[i].userid;
	...
	// Assign to namespace
	namespace('pbskids').Tracker = Tracker;
